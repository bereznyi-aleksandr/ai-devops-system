...
class RuntimeEngine:
    ...
    def __init__(self):
        ...
        self.allowed_repo_operation_statuses = (
            "not_started",
            "blocked",
            "executing",
            "written",
        ...
        self.repo_operation_type = "controlled_write"
        self.repo_operation_status = "not_started"
        self.repo_operation_deny_reason = None
        self.repo_operation_started = None
        self.repo_operation_finished = None
        self.repo_operation_confirmed = False
        self.repo_operation_target = None
        self.repo_operation_commit_sha = None
        self.repo_operation_commit_url = None
        self.repo_operation_previous_file_sha = None
        self.repo_operation_new_file_sha = None
        self.github_branch_head_sha_before_write = None
        self.github_branch_head_sha_after_write = None
    ...
    def _reset_repo_operation_state(self):
        self.repo_operation_type = "controlled_write"
        self.repo_operation_status = "not_started"
        self.repo_operation_deny_reason = None
        self.repo_operation_started = None
    ...
    def prepare_repo_write_plan(self):
        ...
        self._reset_repo_operation_state()
        ...
        if proposed_content is None:
            ...
            self.repo_operation_status = "blocked"
            self.repo_operation_deny_reason = "repo_operation_preconditions_failed"
            print("Repo write status:", self.repo_file_write_status)
            print("Repo write deny reason:", self.repo_file_write_deny_reason)
            print("Repo operation status:", self.repo_operation_status)
            print("Repo operation deny reason:", self.repo_operation_deny_reason)
            return
        ...
        self.repo_operation_target = self.repo_target
    ...
    def execute_repo_write_if_allowed(self):
        ...
        if self.repo_write_approval != "granted":
            ...
            self.repo_operation_status = "blocked"
            self.repo_operation_deny_reason = "repo_operation_not_authorized"
            print("Repo write status:", self.repo_file_write_status)
            print("Repo write deny reason:", self.repo_file_write_deny_reason)
            print("Repo operation status:", self.repo_operation_status)
            print("Repo operation deny reason:", self.repo_operation_deny_reason)
        ...
        self.repo_operation_started = datetime.utcnow().isoformat() + "Z"
        self.repo_operation_status = "executing"
        self.repo_operation_target = self.repo_target
        self.repo_operation_previous_file_sha = self.repo_file_sha
        self.github_branch_head_sha_before_write = self.github_branch_head_sha
        ...
        try:
            ...
            self.repo_operation_status = "written"
            self.repo_operation_commit_sha = self.repo_file_write_commit_sha
            self.repo_operation_commit_url = self.repo_file_write_commit_url
            self.repo_operation_new_file_sha = self.repo_file_sha
            ...
            if e.code == 409:
                ...
                self.repo_operation_deny_reason = "repo_operation_github_conflict"
            else:
                self.repo_file_write_deny_reason = "github_write_failed"
                self.repo_operation_deny_reason = "repo_operation_github_failed"

            self.repo_operation_status = "failed"
            self.repo_operation_finished = datetime.utcnow().isoformat() + "Z"
    ...
    def confirm_repo_write_if_needed(self):
        print("REPO OPERATION: confirming result")
        self.repo_action_class = "repo_operation_confirm"

        if self.repo_operation_status != "written":
            print("Repo operation confirmation skipped")
            return

        if not self.repo_file_write_commit_sha:
            self.repo_operation_status = "failed"
            self.repo_operation_deny_reason = "repo_operation_not_confirmed"
            self.repo_operation_finished = datetime.utcnow().isoformat() + "Z"
            print("Repo operation status:", self.repo_operation_status)
            print("Repo operation deny reason:", self.repo_operation_deny_reason)
            return
        ...
        try:
            confirm_payload = self._github_json(
                "GET",
                f"{repo_base}/contents/{urllib.parse.quote(self.repo_target, safe='/')}"
                f"?ref={urllib.parse.quote(self.repo_file_write_commit_sha, safe='')}",
            )

            if confirm_payload.get("type") != "file":
                self.repo_operation_status = "failed"
                self.repo_operation_deny_reason = "repo_operation_confirmation_failed"
                self.repo_operation_finished = datetime.utcnow().isoformat() + "Z"
                print("Repo operation status:", self.repo_operation_status)
                print("Repo operation deny reason:", self.repo_operation_deny_reason)
                return

            confirm_encoded = confirm_payload.get("content", "")
            confirm_sha = confirm_payload.get("sha")

            if not confirm_encoded or not confirm_sha:
                self.repo_operation_status = "failed"
                self.repo_operation_deny_reason = "repo_operation_confirmation_failed"
                self.repo_operation_finished = datetime.utcnow().isoformat() + "Z"
                print("Repo operation status:", self.repo_operation_status)
            ...
            cleaned = confirm_encoded.replace("\n", "").replace("\r", "").replace(" ", "")
            confirmed_content = base64.b64decode(cleaned).decode("utf-8")

            if "# AGENT_WRITE_TIMESTAMP = " not in confirmed_content:
                self.repo_operation_status = "failed"
                self.repo_operation_deny_reason = "repo_operation_not_confirmed"
                self.repo_operation_finished = datetime.utcnow().isoformat() + "Z"
                print("Repo operation status:", self.repo_operation_status)
            ...
            if confirm_sha == self.repo_operation_previous_file_sha:
                self.repo_operation_status = "failed"
                self.repo_operation_deny_reason = "repo_operation_not_confirmed"
                self.repo_operation_finished = datetime.utcnow().isoformat() + "Z"
                print("Repo operation status:", self.repo_operation_status)
            ...
            self.github_branch_head_sha_after_write = branch_payload.get("commit", {}).get("sha")

            self.repo_file_content = confirmed_content
            self.repo_file_sha = confirm_sha
            self.repo_file_size_bytes = len(confirmed_content.encode("utf-8"))
            self.repo_file_line_count = len(confirmed_content.splitlines())
            self.repo_file_content_loaded = True

            self.repo_operation_status = "confirmed"
            self.repo_operation_confirmed = True
            self.repo_operation_new_file_sha = confirm_sha
            self.repo_operation_finished = datetime.utcnow().isoformat() + "Z"

            print("Repo operation status:", self.repo_operation_status)
            print("Repo operation confirmed:", str(self.repo_operation_confirmed).lower())
            print("Repo operation new file sha:", self.repo_operation_new_file_sha)
            print("GitHub branch head sha before write:", self.github_branch_head_sha_before_write)
            print("GitHub branch head sha after write:", self.github_branch_head_sha_after_write)

        except Exception as e:
            self.repo_operation_status = "failed"
            self.repo_operation_deny_reason = "repo_operation_confirmation_failed"
            self.repo_operation_finished = datetime.utcnow().isoformat() + "Z"
            print("Repo operation confirmation failed:", e)
            print("Repo operation status:", self.repo_operation_status)
            print("Repo operation deny reason:", self.repo_operation_deny_reason)
    ...
    def build_history_record(
        ...
        return {
            ...
            "repo_operation_type": self.repo_operation_type,
            "repo_operation_status": self.repo_operation_status,
            "repo_operation_deny_reason": self.repo_operation_deny_reason,
            "repo_operation_started": self.repo_operation_started,
            "repo_operation_finished": self.repo_operation_finished,
            "repo_operation_confirmed": self.repo_operation_confirmed,
            "repo_operation_target": self.repo_operation_target,
            "repo_operation_commit_sha": self.repo_operation_commit_sha,
            "repo_operation_commit_url": self.repo_operation_commit_url,
            "repo_operation_previous_file_sha": self.repo_operation_previous_file_sha,
            "repo_operation_new_file_sha": self.repo_operation_new_file_sha,
            "github_branch_head_sha_before_write": self.github_branch_head_sha_before_write,
            "github_branch_head_sha_after_write": self.github_branch_head_sha_after_write,
        }
    ...
    def run(self):
        ...
        self.confirm_repo_write_if_needed()
