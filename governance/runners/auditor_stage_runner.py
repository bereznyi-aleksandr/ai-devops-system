...
def handle_pre(contour_id: str, task: dict) -> str:
    ...
    routed = {
        ...
        "status": "pending" if ok else "rejected", "from": role_name, "to": f"{contour_id}.EXECUTOR",
        "contour_id": contour_id, "phase": "execute", "pre_check": "PASS" if ok else "FAIL",
        "plan_path": task.get("plan_path"), "task": task.get("task") or "", "created_at": now_iso(),
    }
   if ok:
...
def handle_post(contour_id: str, task: dict) -> str:
    ...
    feedback = {
        ...
        "status": "completed", "from": role_name, "to": "WRK.CURATOR", "contour_id": contour_id,
        "phase": "post_check", "acceptance": "PASS" if ok else "FAIL", "result_path": task.get("result_path"), "created_at": now_iso(),
    }
