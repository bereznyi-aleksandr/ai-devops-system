...
p = Path('governance/runtime/packets/auditor_packet_current.json')
...
write_failure() {
  local details="$1"
  python3 "$FAILURE_WRITER" AUDITOR "$TASK_ID" "$details" || true
}
...
if ! codex --full-auto "$CODEX_PROMPT"; then
  write_failure "Codex auditor run failed"
  exit 1
fi
...
required = {
    ...
    'reviewed_ref','reviewed_commit_sha','summary','rationale','conditions',
    'blocked_reason','touched_files','ts_utc'
}
