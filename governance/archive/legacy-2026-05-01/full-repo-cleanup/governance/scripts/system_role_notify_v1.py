...
def main() -> int:
    ...
    role = str(target.get('notify_role', '')).strip()
    ...
    active_path = role_dir / f'{task_id}.notify.json'
    if active_path.exists():
        print(json.dumps({
            ...
            'notification_created': False,
            'notification_path': str(active_path.relative_to(ROOT)),
            'reason': 'active_notify_exists',
        }, ensure_ascii=False, indent=2))
        return 0
