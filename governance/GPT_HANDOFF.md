...
```
BEM-<номер> | <НАЗВАНИЕ ЗАДАЧИ> | YYYY-MM-DD | <XX%>
```

**Процентовка** — прогресс выполнения текущей задачи:
- `0%` — только начал читать/анализировать
- `25%` — план готов, к выполнению не приступал
- `50%` — в процессе выполнения
- `75%` — выполнено, пишу отчёт/коммичу
- `100%` — полностью готово, доказательство есть

---

### Чек-лист (сразу после заголовка)

```
Чек-лист:
✅ <что сделано успешно>
✅ <что сделано успешно>
⚠️ <что сделано частично или требует внимания>
❌ <что не сделано / blocker>
```

Правила чек-листа:
- Минимум 3 пункта, максимум 8
- Каждый пункт — одно конкретное действие или факт
- ✅ только если есть доказательство (файл создан, коммит сделан)
- ❌ только если есть точный blocker с причиной

---

### Табличная часть (детальный пошаговый отчёт)

```
| Тип                | Содержание                                      | Статус  |
|--------------------|------------------------------------------------|---------|
| Факт               | <что прочитал / обнаружил>                      | INFO    |
| Факт               | <что изменил / создал>                          | DONE    |
| Факт               | <commit SHA если был коммит>                    | DONE    |
| Blocker            | <точная причина если есть>                      | BLOCKED |
| Следующее действие | <один конкретный следующий шаг>                 | NEXT    |
```

Типы строк:
- `Факт` — что произошло (нейтральный)
- `Blocker` — что мешает продолжать (обязательна точная причина)
- `Следующее действие` — один шаг, не список

---

### Код-блок (финал каждого ответа)

```
STATUS: completed | blocked | failed | in_progress
NEXT: <один следующий шаг или "ожидаю оператора">
BLOCKER: none | <точная причина>
COMMIT_SHA: none | <sha>
CHANGED_FILES: none | <список файлов>
```
...
```
BEM-500 | FIX ROLE ORCHESTRATOR | 2026-05-15 | 100%

Чек-лист:
✅ Прочитал governance/GPT_HANDOFF.md — контекст восстановлен
✅ Прочитал .github/workflows/role-orchestrator.yml — найдена проблема
✅ Исправил YAML: убран schedule trigger
✅ Коммит сделан: abc1234
❌ Self-test не запущен — нет active runner

| Тип                | Содержание                                           | Статус  |
|--------------------|-----------------------------------------------------|---------|
| Факт               | role-orchestrator.yml содержал schedule: cron        | INFO    |
| Факт               | Убран schedule, оставлен workflow_dispatch only      | DONE    |
| Факт               | Commit SHA: abc1234def567                            | DONE    |
| Blocker            | Self-test требует manual workflow_dispatch от оператора | BLOCKED |
| Следующее действие | Оператор запускает role-orchestrator mode=start      | NEXT    |

STATUS: completed
NEXT: оператор запускает role-orchestrator workflow_dispatch
BLOCKER: none
COMMIT_SHA: abc1234def567
CHANGED_FILES: .github/workflows/role-orchestrator.yml
```
...
- YAML fixes ✅ (BEM-488)
- GPT_HANDOFF.md создан ✅

### P1 — В работе
- GPT автономность через GitHub MCP
...
7. Написать результат по КАНОНУ оформления (см. выше)
...
*Последнее обновление: 2026-05-15 v1.1 | Claude Sonnet 4.6*
